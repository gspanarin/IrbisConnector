...
class Irbis64Server { 
    ...
    function record_write($db_name, &$record, $lock = false, $if_update = true) { // $lock  = блокировать запись , $if_update = актуализировать запись
        ...
        if (array_key_exists('entry',$this->ini)) { 
            ...
            foreach($flc as $value){
                if (mb_substr($value,0,1) != '0' and trim($value) <> ''){
                    $this->error_code = '-11111';
   ...
   **/
   ...
   function getresourse($path,$db_name,$filename){
      $packet = implode("\n", array('L', $this->arm, 'L', $this->id, $this->seq, '', '', '', '', '', $path.'.'.$db_name.'.'.$filename));
      $packet = strlen($packet) . "\n" . $packet;
      $answer = $this->send($packet);
   ...
   function getresourse2($path,$db_name,$filename){
      $packet = implode("\n", array('L', $this->arm, 'L', $this->id, $this->seq, '', '', '', '', '', $path.'.'.$db_name.'.'.$filename));
      $packet = strlen($packet) . "\n" . $packet;
      $answer = $this->send($packet);
...
class irbis64_user{
   ...
   public $a;
	...
	function __construct($login, $password,$c = 'irbisc.ini',$r = 'irbisr.ini',$b = 'irbisb.ini',$p = 'irbisp.ini',$k = 'irbisk.ini',$a = 'irbisa.ini') {
